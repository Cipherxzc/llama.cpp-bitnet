...
static void ggml_compute_forward_mul_mat(
    ...
    // if (vec_dot_type == GGML_TYPE_I8_SS){
    //     printf("%d, %d\n", params->wsize, src1->ne[1] * src1->ne[2] * src1->ne[3] * ggml_row_size(vec_dot_type,
    //     src1->ne[0]));
    // }
...
UseGgmlGemm1:;
    ...
    if (src1->type != vec_dot_type) { // 将量化后的 data 存至 params->wdata 中
        ...
        const size_t nbw1 = ggml_row_size(vec_dot_type, ne10);  // vec_dot_type 的 row_size
